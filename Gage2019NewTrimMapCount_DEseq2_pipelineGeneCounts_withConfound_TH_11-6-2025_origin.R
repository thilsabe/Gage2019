...
save_venn_plot <- function(gene_lists, title, filename, base_fill_color = "blue") {
  ...
  p <- ggVennDiagram(gene_lists, label_alpha = 0) +
    ...
    # Expand x-axis limits to prevent label clipping
...
get_symbols <- function(gene_ids, gene_map) {
  tibble(ensembl_gene_id = gene_ids) %>%
    left_join(gene_map, by = "ensembl_gene_id")
}

# Save gene lists and overlaps including gene symbols
save_gene_lists_and_overlap <- function(gene_lists, comparison_name, fill_cols, venn_dir, overlap_dir) {
  # Combine all gene lists, annotate with symbols
  gene_lists_df <- gene_lists %>%
  ...
  # Filter out empty or "No significant DEGs" lists
  valid_lists <- gene_lists[sapply(gene_lists, function(x) length(x) > 0 && all(x != "No significant DEGs"))]
  
  if (length(valid_lists) > 1) {
    overlap_genes <- Reduce(intersect, valid_lists)
  } else {
    overlap_genes <- character(0)
  }
  
  # Annotate overlap genes with symbols
  overlap_annotated <- get_symbols(overlap_genes, gene_map)
  ...
  # Save overlap genes as one-row summary CSV
  overlap_row <- tibble(
  ...
  save_venn_plot(gene_lists, paste("DEGs by", gsub("_", " ", comparison_name)), png_file)
}
...
for (t in names(venn_by_trimmer_counter)) {
  gene_lists <- venn_by_trimmer_counter[[t]]
  if (length(gene_lists) > 1 && sum(sapply(gene_lists, length) > 0) >= 2) {
    # fill_cols <- c("red", "green", "blue")[seq_along(gene_lists)]
    save_gene_lists_and_overlap(gene_lists,
                                paste0("counters_within_", t),
                                fill_cols,
                                venn_dir,
                                overlap_dir)
  }
...
for (a in names(venn_by_aligner_counter)) {
  gene_lists <- venn_by_aligner_counter[[a]]
  if (length(gene_lists) > 1 && sum(sapply(gene_lists, length) > 0) >= 2) {
    # fill_cols <- c("orange", "purple", "cyan")[seq_along(gene_lists)]
    save_gene_lists_and_overlap(gene_lists,
                                paste0("trimmers_within_", a),

...
priorityFlag <- FALSE # set TRUE to only use deg sets in priority_deg_sets list
...
# ============================================================
# 3. LOAD DEG FILE (col 1 = varying dimension, last col = gene)
# ============================================================
# FIX: the varying column is not always "aligner" — for trimmer
# or counter DEG sets col 1 holds trimmer/counter values.
# We rename it to `varying_dim` generically so the rest of the
# pipeline can use meta$grouping_type to interpret it without
# needing to rename 40+ downstream references.
# The column `aligner` is ONLY populated when grouping_type=="aligner".
# For trimmer/counter sets it stays NA and the GLMM uses the
# appropriate vary_col resolved from grouping_type.

load_deg_file <- function(deg_file, grouping_type) {
  dt          <- fread(deg_file)
  varying_col <- names(dt)[1]          # col 1 = whatever is varying
  gene_col    <- names(dt)[ncol(dt)]   # last col = gene ID

  cat(sprintf("  DEG file col1='%s' (grouping=%s) | last col='%s'\n",
              varying_col, grouping_type, gene_col))

  out <- unique(dt[, .(
    varying_dim = get(varying_col),
    ensembl_id  = get(gene_col)
  )])

  # Also populate the named dimension column so downstream
  # merge(by="ensembl_id") and split(by="aligner") still work
  # without any other code changes.
  if (grouping_type == "aligner") {
    out[, aligner          := varying_dim]
    out[, deg_list_trimmer := NA_character_]
    out[, counter_varying  := NA_character_]
  } else if (grouping_type == "trimmer") {
    out[, aligner          := NA_character_]
    out[, deg_list_trimmer := varying_dim]
    out[, counter_varying  := NA_character_]
  } else if (grouping_type == "counter") {
    out[, aligner          := NA_character_]
    out[, deg_list_trimmer := NA_character_]
    out[, counter_varying  := varying_dim]
  } else {
    out[, aligner          := NA_character_]
    out[, deg_list_trimmer := NA_character_]
    out[, counter_varying  := NA_character_]
...
for (deg_file in deg_files) {
  ...
  # ----------------------------------------------------------
  deg_dt <- load_deg_file(deg_file, meta$grouping_type)
  ...
  # Join on ensembl_id to bring the varying-dim value into each kmer row.
  # We keep the column as `aligner` in merged so all downstream code
  # (UpSet grouping, all_kmer_aligner_lists, etc.) works unchanged.
  # For trimmer/counter DEG sets `aligner` will hold the trimmer/counter
  # value — that is intentional: it represents "which dim-value called
  # this gene as a DEG", which is what every downstream split() needs.
  deg_join_cols <- unique(deg_dt[, .(ensembl_id, varying_dim)])
  setnames(deg_join_cols, "varying_dim", "aligner")   # generic rename
  ...
  cat("  merged rows after varying-dim join:", nrow(merged), "\n")
  ...
  # ----------------------------------------------------------
  # BUILD GLMM INPUT WITH BINARY RESPONSE
  # Root cause of "Response is constant": merged only contains
  # kmers whose gene was called as a DEG, so DEG_binary=1 for
  # all rows. The GLMM needs both 1 (kmer in DEG gene, for this
  # varying-dim value) and 0 (kmer NOT in any DEG gene, or kmer
  # in DEG gene for a different dim value).
  #
  # Strategy: start from kmer_dt (all sampled kmers, including
  # background non-sig), cross with each unique varying-dim
  # value, then assign DEG_binary=1 only where the kmer's gene
  # appears in deg_dt for that specific dim value.
  # ----------------------------------------------------------

  vary_col_name <- switch(meta$grouping_type,
    "aligner" = "aligner",
    "trimmer" = "deg_list_trimmer",
    "counter" = "counter_varying",
    NULL
  )

  # The unique values the varying dimension takes in this DEG set
  dim_values <- unique(deg_dt$varying_dim[!is.na(deg_dt$varying_dim)])
  cat(sprintf("  Varying dim '%s': %d values (%s)\n",
              meta$grouping_type, length(dim_values),
              paste(dim_values, collapse=", ")))

  # Cross kmer_dt with all dim values to get one row per kmer x dim
  # then flag DEG_binary based on whether (ensembl_id, dim_val) is in deg_dt
  kmer_dt_glmm <- kmer_dt[!is.na(ensembl_id)]

  if (length(dim_values) > 0) {
    # Build a lookup set: which (ensembl_id, varying_dim) pairs are DEGs?
    deg_lookup <- unique(deg_dt[, .(ensembl_id, varying_dim)])
    setkey(deg_lookup, ensembl_id, varying_dim)

    # Cross-join: one row per kmer x dim_value
    dim_dt <- data.table(varying_dim = dim_values)
    glmm_base <- kmer_dt_glmm[, .(
      kmer         = kmer,
      ensembl_id   = ensembl_id,
      fastq_trimmer = fastq_trimmer,
      kmer_entropy = if ("kmer_entropy" %in% names(kmer_dt_glmm)) kmer_entropy else NA_real_,
      gc_content   = if ("gc_content"   %in% names(kmer_dt_glmm)) gc_content   else NA_real_
    )]
    # deduplicate kmer x fastq_trimmer first (many-to-one from gene annotation)
    glmm_base <- unique(glmm_base)

    glmm_crossed <- glmm_base[, .(varying_dim = dim_values), by=.(kmer, ensembl_id,
                                                                    fastq_trimmer,
                                                                    kmer_entropy,
                                                                    gc_content)]
    glmm_crossed[, DEG_binary := as.integer(
      paste(ensembl_id, varying_dim, sep="___") %in%
        paste(deg_lookup$ensembl_id, deg_lookup$varying_dim, sep="___")
    )]

    # Rename varying_dim back to the pipeline-dimension column name
    # so run_glmm_analysis can find it via vary_col
    if (meta$grouping_type == "aligner") {
      setnames(glmm_crossed, "varying_dim", "aligner")
    } else if (meta$grouping_type == "trimmer") {
      setnames(glmm_crossed, "varying_dim", "deg_list_trimmer")
    } else if (meta$grouping_type == "counter") {
      setnames(glmm_crossed, "varying_dim", "counter")
    }

    n_pos <- sum(glmm_crossed$DEG_binary == 1L)
    n_neg <- sum(glmm_crossed$DEG_binary == 0L)
    cat(sprintf("  GLMM binary response: %d positive (DEG) | %d negative (non-DEG)\n",
                n_pos, n_neg))

    if (n_pos == 0 || n_neg == 0) {
      cat("  [WARNING] GLMM response still constant after cross-join — check deg_dt overlap\n")
      cat("  deg_dt ensembl_ids (first 5):", head(deg_dt$ensembl_id, 5), "\n")
      cat("  kmer_dt ensembl_ids (first 5):", head(kmer_dt_glmm$ensembl_id, 5), "\n")
    }
  } else {
    cat("  [WARNING] No varying dim values found — GLMM input will be empty\n")
    glmm_crossed <- data.table()
  }
  ...
  if (n_sig > 0) {
    ...
    paths <- kmer_paths[[names(kmer_paths)[1]]]
  ...
  # Uses glmm_crossed built above — one row per kmer x varying-dim
  # value with correct DEG_binary (0 or 1). This is the key fix:
  # kmers_dt (merged) only has DEG-matched rows so DEG_binary=1
  # everywhere. glmm_crossed has the full kmer set crossed against
  # all varying-dim values with proper binary labels.
  # -------------------------------------------------------
  cat("  Running per-DEG-set GLMM analysis...\n")

  if (exists("glmm_crossed") && is.data.table(glmm_crossed) && nrow(glmm_crossed) > 0) {
    cat(sprintf("  GLMM rows: %d (DEG=%d | BG=%d)\n",
                nrow(glmm_crossed),
                sum(glmm_crossed$DEG_binary == 1L),
                sum(glmm_crossed$DEG_binary == 0L)))
    ...
    cat("  [SKIP] GLMM — glmm_crossed is empty or missing\n")
  }
